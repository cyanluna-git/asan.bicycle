...
export async function loadExplorePageData({
  params,
  selectedCourseIdOverride = null,
}: {
  params: ExploreSearchParams
  selectedCourseIdOverride?: string | null
}) {
  ...
  const startPointList = (startPointsResult.data ?? []).map((sp) => ({
    id: sp.id,
    name: sp.name,
  }))
  ...
  if (coursesError) {
    console.error('[explore-data] courses error:', coursesError.message, coursesError.details)
  }
  ...
  const selectedCourseId = selectedCourseIdOverride
